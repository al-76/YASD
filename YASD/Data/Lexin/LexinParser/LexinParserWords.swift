...
class LexinParserWordsDefault: LexinParserWords {
    ...
    private static let soundUrl = "https://lexin.nada.kth.se/sound/"
    ...
    private func getRequestParameters(_ word: String, _ language: String) -> (String?, [String: String]?) {
        let body = "7|0|7|" + LexinParserWordsDefault.url +
            "|FCDCCA88916BAACF8B03FB48D294BA89|se.jojoman.lexin.lexingwt.client.LookUpService|" +
            "lookUpWord|se.jojoman.lexin.lexingwt.client.LookUpRequest/682723451|" +
            language + "|" +
    ...
    fileprivate static func parseSoundUrl(_ root: String, with element: HtmlParserElement) throws -> String? {
        ...
        if let res = try? phonetic?.attribute("File").removeQuotes() {
            return soundUrl + res.replaceNonAscii()
        }
        return nil
    }
...
class LexinParserWordsSwedish: LexinParserWordsDefault {
    ...
    private static func parseLexems(_ id: String, with element: HtmlParserElement) throws -> [LexinWord.Lang] {
        ...
        if langs.isEmpty {
            ...
            langs.append(LexinWord.Lang(meaning: nil, phonetic: phonetic,
                                        inflection: inflection, grammar: nil,
                                        example: nil, idiom: nil,
                                        compound: nil, translation: nil,
                                        reference: reference, synonym: nil,
                                        soundUrl: soundUrl))
        }
        return langs
    }
...
private extension String {
    ...
    func replaceNonAscii() -> String {
        return replacingOccurrences(of: "ä", with: "0344")
...
class LexinParserWordsFolkets: LexinParserWords {
    ...
    private func getRequestparameters(_ word: String, _: String) -> (String?, [String: String]?) {
        let body = "7|0|6|" + LexinParserWordsFolkets.url +
            "|1F6DF5ACEAE7CE88AACB1E5E4208A6EC|se.algoritmica.folkets.client.LookUpService|" +
            "lookUpWord|se.algoritmica.folkets.client.LookUpRequest/1089007912|" +
    ...
    private static func parseTranslatedLang(_ element: HtmlParserElement) throws -> LexinWord.Lang {
        ...
        let lang = LexinWord.Lang(meaning: nil, phonetic: nil,
                                  inflection: nil, grammar: nil,
                                  example: example, idiom: nil,
                                  compound: nil, translation: nil,
                                  reference: nil, synonym: nil,
                                  soundUrl: nil)
        return lang
    }
    ...
    private static func parseSoundUrl(element: HtmlParserElement) throws -> String? {
        ...
        if let res = try? phonetic?.attribute("soundFile").removeQuotes() {
            return soundUrl + res.replacingOccurrences(of: "swf", with: "mp3").replaceNonAscii()
        }
        return nil
    }
}
...
private func parsePhonetic(_ phonetic: String) -> String {
    ...
    if let regex = try? NSRegularExpression(pattern: "\\\\u([0-9a-fA-F]{1,4})", options: []) {
        for match in regex.matches(in: res, options: [], range: NSRange(location: 0, length: res.count)) {
